...
f = vle.recursive.init(pkg="AZODYN_avec_stress_T_et_H", file="AzodynWheat.vpz")
...
if (Donnees_sinistres$Sinistre[1] == "thermique"){ f = vle.recursive.init(pkg="AZODYN_avec_stress_thermique", file="AzodynWheat.vpz")}else{f = vle.recursive.init(pkg="AZODYN_stress_H", file="AzodynWheat.vpz")}

write.csv2(meteo_substract,file ="meteo_substract.csv",row.names = FALSE)
...
dscinistre = data.frame(res$RDT,res$MS)
colnames(dscinistre)=c("RDTreel","MSreel")


dscinistre
write.csv2(dscinistre,file = "dscinistre.csv",row.names = FALSE)
